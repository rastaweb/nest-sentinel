...
describe('IPValidator', () => {
  ...
  describe('isIPInRange', () => {
    it('should validate IP addresses in CIDR ranges', () => {
      expect(IPValidator.isIPInRange('192.168.1.10', '192.168.1.0/24')).toBe(true);
      expect(IPValidator.isIPInRange('10.0.0.50', '10.0.0.0/8')).toBe(true);
      expect(IPValidator.isIPInRange('172.16.5.10', '172.16.0.0/12')).toBe(true);
    });

    it('should reject IP addresses outside CIDR ranges', () => {
      expect(IPValidator.isIPInRange('192.168.2.10', '192.168.1.0/24')).toBe(false);
...
describe('APIKeyValidator', () => {
  describe('extractAPIKey', () => {
    it('should extract API key from default header', () => {
      ...
      const query = {};
      expect(APIKeyValidator.extractAPIKey(headers, query)).toBe('test-key-123');
    });

    it('should extract API key from custom header', () => {
      const headers = { 'authorization': 'Bearer test-key-123' };
      const query = {};
      expect(APIKeyValidator.extractAPIKey(headers, query, { header: 'authorization' })).toBe('Bearer test-key-123');
    });
    ...
    it('should extract API key from custom query parameter', () => {
      ...
      expect(APIKeyValidator.extractAPIKey(headers, query, { query: 'token' })).toBe('test-key-123');
    });
  ...
  describe('validateWithMetadata', () => {
    ...
    it('should return valid for valid metadata', () => {
      const metadata = { name: 'test-key', maxRequests: 100, currentRequests: 50 };
      const result = APIKeyValidator.validateWithMetadata('test-key', metadata);
